<!DOCTYPE html>
<html>

<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="initial-scale=1, width=device-width" />
  <meta name="viewport" content="initial-scale=1, width=device-width" />
  <link rel="stylesheet" href="globals.css" />
  <link rel="stylesheet" href="styleguide.css" />
  <link rel="stylesheet" href="style.css" />
</head>

<body>
  <?php
  include '../../Template/navbar.php';
  ?>
  <div class="my-projects">
    <div class="div">
      <div class="text-wrapper">My Projects</div>
      <div class="overlap">
        <img class="rectangle" src="img/rectangle-32.png" alt="new" />
        <div class="text-wrapper-2">Graduation Party</div>
        <div class="frame">
          <div class="text-wrapper-3">$8</div>
          <div class="text-wrapper-4">$45</div>
        </div>
        <div class="text-wrapper-5">Includes taxes and fees</div>
        <div class="text-wrapper-6">4.5 (1200 Reviews)</div>
        <div class="div-wrapper">
          <div class="text-wrapper-7">View party details</div>
        </div>
        <img class="group" src="img/group-5.png" alt="new" />
        <div class="frame-2">
          <div class="text-wrapper-8">Non refundable</div>
          <p class="p">Date: Sunday, March 18, 2022</p>
        </div>
      </div>
      <div class="div-2">
        <div class="div-2">
          <div class="header">
            <div class="logo-header">
              <div class="logo">
                <div class="text-wrapper-9">PartyPRO</div>
                <div class="pattern"></div>
                <div class="pattern-2"></div>
                <div class="pattern-3"></div>
                <div class="pattern-4"></div>
              </div>
            </div>
          </div>
        </div>
        <div class="menu">
          <div class="menu-2">
            <div class="text-wrapper-10">Home</div>
            <div class="about">
              <div class="text-wrapper-11">About</div>
              <img class="line" src="img/line-4-1.svg" alt="new" />
            </div>
            <div class="pages">
              <div class="pages-2">
                <div class="text-wrapper-12">Pricing</div>
              </div>
              <img class="line" src="img/line-4-1.svg" alt="new" />
            </div>
            <div class="project">
              <div class="text-wrapper-11">My Projects</div>
              <img class="line" src="img/line-4-1.svg" alt="new" />
            </div>
            <div class="contact">
              <div class="text-wrapper-11">Features</div>
              <img class="line" src="img/line-4-1.svg" alt="new" />
            </div>
            <div class="contact-2">
              <div class="text-wrapper-11">SignIn</div>
              <img class="line" src="img/line-4-1.svg" alt="new" />
            </div>
          </div>
        </div>
      </div>
      <footer class="footer">
        <div class="overlap-group">
          <div class="background"></div>
          <img class="img" src="img/background-1.svg" alt="new" />
          <div class="copyright">
            <img class="line-2" src="img/line.svg" alt="new" />
            <div class="copyright-partypro-wrapper">
              <p class="copyright-partypro">
                <span class="span">Copyright © PartyPRO | Designed by XXX - Powered by
                  XXX.</span>
              </p>
            </div>
          </div>
          <div class="content">
            <div class="about-us-our-team-wrapper">
              <p class="about-us-our-team">
                About Us <br />Our Team <br />Our Project<br />Pricing
                <br />Contact
              </p>
            </div>
            <div class="subscribe">
              <div class="content-2">
                <div class="name">
                  <div class="overlap-group-2">
                    <div class="text-wrapper-13">Email here*</div>
                  </div>
                </div>
                <button class="button">
                  <button class="button-2">
                    <div class="text-wrapper-14">Send Now</div>
                  </button>
                  <img class="follow" src="img/follow.svg" alt="new" />
                </button>
              </div>
            </div>
            <div class="logo-2">
              <div class="text-wrapper-9">PartyPRO</div>
              <div class="pattern"></div>
              <div class="pattern-2"></div>
              <div class="pattern-3"></div>
              <div class="pattern-4"></div>
            </div>
            <div class="content-3">
              <p class="join-the-partypro">
                Join the PartyPRO community for exclusive tips, inspiration,
                and offers to make your celebrations truly unforgettable.
                Let&#39;s create memories together!
              </p>
              <div class="address">
                <div class="email">
                  <a href="mailto:contact@hvcargologistics.com" target="_blank" rel="noopener noreferrer">
                    <div class="text-wrapper-15">
                      Email<br />contact@partyprocom
                    </div>
                  </a>
                  <div class="icon">
                    <div class="vector-wrapper">
                      <img class="vector" src="img/vector-1.svg" alt="new" />
                    </div>
                  </div>
                </div>
                <div class="call">
                  <p class="text-wrapper-15">
                    Call Us <br />(00) 112 365 489
                  </p>
                  <div class="icon">
                    <img class="vector-2" src="img/vector-2.svg" alt="new" />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <img class="image" src="img/image.png" alt="new" />
        </div>
      </footer>
    </div>
  </div>
</body>

</html>